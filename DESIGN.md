...
The intended execution and implementation tooling is Claude Code with Opus and a browser harness such as Browserbase. Those adapters are not implemented by the website redesign. Existing runtime identifiers and evidence permissions remain stable.

## Interface

Use warm ivory surfaces, a charcoal navigation sidebar, olive actions, and restrained lime highlights. Show real investigations separately from the explicitly labeled scripted sample. Give the swarm the most space in the overview diagram. Keep the full evidence ledger, audit export, event stream, rollback history, browser sessions, and human decisions in each investigation.

Customer intake produces a downloadable brief. Saving a brief in the browser does not start the reproduction runtime. The interface must state this boundary. Infrastructure failures are never represented as evidence that the product is broken.
